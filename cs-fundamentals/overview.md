...
```
Computer Science
+-- Theory of Computation -------> [[discrete-math|离散数学]]
|   +-- Automata Theory
|   +-- Computability
|   +-- Complexity Theory
+-- Computer Architecture -------> [[architecture|体系结构]]
|   +-- ISA Design
|   +-- Pipeline & Superscalar
|   +-- Memory Hierarchy
+-- Systems Software -----------> [[os|操作系统]] / [[compiler|编译原理]]
|   +-- Process Management
|   +-- Memory Management
|   +-- Code Generation
+-- Networking -----------------> [[network|计算机网络]]
|   +-- Protocol Stack
|   +-- Routing & Switching
|   +-- Application Protocols
+-- Programming Languages ------> [[compiler|编译原理]] / [[design-patterns|设计模式]]
|   +-- Syntax & Semantics
|   +-- Type Systems
|   +-- Paradigms (OOP/FP/LP)
+-- Algorithms & DS ------------> (贯穿全模块)
...
|   +-- Graph Algorithms
|   +-- NP-Completeness
+-- Software Engineering -------> [[design-patterns|设计模式]]
    +-- Design Patterns
...
> 跨模块引用：[[c/overview|C语言]]的指针操作直接穿透了语言运行时层到达指令集层，这是C语言"接近硬件"的本质原因。[[cpp/overview|C++]]的RAII则在语言运行时层提供了资源管理的抽象。
...
> 跨模块引用：[[discrete-math|离散数学]]中的自动机理论是状态机主线的数学基础。[[java/overview|Java]]线程的状态模型与OS进程状态机直接对应。
...
| 理论结果 | 影响的模块 | 具体影响 |
|----------|------------|----------|
| 停机问题不可判定 | [[compiler|编译原理]] | 无法构建完美的程序分析器 |
| Chomsky层次 | [[compiler|编译原理]] | 正则语言/上下文无关语言决定词法/语法分析的能力边界 |
| NP-completeness | [[design-patterns|设计模式]] | 某些优化问题需要启发式/近似算法 |
| 信息熵 | [[network|计算机网络]] | 信道容量、数据压缩的理论极限 |
| 自动机理论 | [[discrete-math|离散数学]] | DFA/NFA/正则表达式的等价性 |
...
| 本模块章节 | 外部模块引用 | 交叉原因 |
|------------|-------------|----------|
| architecture | [[c/overview\|C语言]] | C的内存模型直接映射到硬件 |
| os | [[cpp/overview\|C++]] | C++的线程模型与OS线程对应 |
| compiler | [[java/overview\|Java]] | JVM是编译目标+运行时的统一体 |
| network | [[java/overview\|Java]] | Java NIO/Netty与网络编程 |
| design-patterns | [[cpp/overview\|C++]] / [[java/overview\|Java]] | 设计模式的语言实现差异 |
