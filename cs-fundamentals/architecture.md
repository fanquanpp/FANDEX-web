...
> 跨模块引用：[[c/overview|C语言]]的函数指针特性直接利用了冯诺依曼体系中"代码即数据"的本质。[[os|操作系统]]的虚拟内存管理通过MMU在冯诺依曼体系上实现了地址空间的隔离。
...
ISA是硬件和软件之间的**接口契约**（参见[[overview|概述]]的抽象层级模型）。ISA定义了：
...
> 跨模块引用：[[compiler|编译原理]]的代码生成阶段需要根据ISA选择合适的寻址模式和指令调度。[[c/overview|C语言]]的指针算术直接映射到基址偏移寻址模式。
...
> 跨模块引用：[[os|操作系统]]的进程上下文切换需要保存/恢复流水线状态。[[compiler|编译原理]]的指令调度需要理解流水线冒险以避免性能损失。
...
> 跨模块引用：[[os|操作系统]]的内存管理建立在虚拟内存机制之上。[[c/overview|C语言]]的指针解引用触发完整的地址翻译链路。[[cpp/overview|C++]]的智能指针在虚拟内存之上增加了语义层。
...
> 跨模块引用：[[cpp/overview|C++]]的内存模型（memory_order_relaxed/acquire/release/seq_cst）直接映射到硬件一致性模型。[[java/overview|Java]]的volatile关键字在x86上无需额外屏障，但在ARM上需要dmb。
