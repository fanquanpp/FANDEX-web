...
> 跨模块引用：[[overview|概述]]的停机问题证明使用了反证法。[[compiler|编译原理]]的类型正确性证明使用了归纳法。[[c/overview|C语言]]的断言(assert)是命题逻辑在编程中的应用。
...
> 跨模块引用：[[os|操作系统]]的进程等价类划分（同组进程）使用了等价关系。[[network|计算机网络]]的子网划分本质上是IP地址集合上的等价关系。[[compiler|编译原理]]的类型等价判断依赖于名字等价或结构等价关系。
...
应用:
  [[compiler|编译原理]]的寄存器分配: 干涉图着色
  [[os|操作系统]]的资源分配: 无死锁检测
  [[network|计算机网络]]的信道分配: 频率分配
```

> 跨模块引用：[[network|计算机网络]]的路由算法（OSPF）基于Dijkstra算法。[[compiler|编译原理]]的寄存器分配使用图着色算法。[[os|操作系统]]的资源分配图用于死锁检测。[[design-patterns|设计模式]]的组合模式形成树结构。
...
> 跨模块引用：[[architecture|体系结构]]的缓存关联度计算使用组合计数。[[compiler|编译原理]]的解析表构造使用容斥原理。[[network|计算机网络]]的子网划分使用组合计数。[[overview|概述]]的复杂性类分析使用递推关系和主定理。
...
> 跨模块引用：[[network|计算机网络]]的RSA/ECC加密建立在数论和群论基础上。[[compiler|编译原理]]的哈希函数使用模运算。[[c/overview|C语言]]的整数溢出行为与模运算直接相关。[[architecture|体系结构]]的ALU实现了模2^n的算术运算。
...
```
Chomsky文法层次 (参见 [[compiler|编译原理]] 8.2节):
...
> 跨模块引用：[[compiler|编译原理]]的词法分析使用DFA/正则表达式，语法分析使用CFG/PDA。[[overview|概述]]的计算理论建立在Chomsky层次之上。[[architecture|体系结构]]的CPU控制单元本质上是有限状态机。
