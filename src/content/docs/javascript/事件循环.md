...
learningObjectives:
  - bloom: remember
    objective: 复述 HTML 规范定义的事件循环处理模型，列举任务源与任务队列
  - bloom: understand
    objective: 解释 JavaScript 单线程模型的设计动机与事件循环的协调机制
  - bloom: apply
    objective: 正确使用 setTimeout、setImmediate、queueMicrotask、requestAnimationFrame 调度任务
  - bloom: analyze
    objective: 拆解复杂异步代码的执行顺序，预测输出（含 await、Promise、microtask）
  - bloom: evaluate
    objective: 评估浏览器与 Node.js 事件循环的差异，选择合适的调度策略
  - bloom: create
    objective: 设计一个支持优先级与背压的任务调度器，集成微任务、宏任务与渲染时机
exercises:
  - type: fill-blank
    bloom: remember
