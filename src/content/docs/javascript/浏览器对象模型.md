...
learningObjectives:
  - bloom: remember
    objective: 复述 BOM 的概念边界与历史成因，区分 BOM 与 DOM 的关系
  - bloom: understand
    objective: 解释 window 对象的三重身份（全局对象、窗口代理、浏览器环境宿主）
  - bloom: apply
    objective: 使用 location、navigator、history、screen 等 API 解决实际工程问题
  - bloom: analyze
    objective: 拆解 postMessage 与 structured clone 算法在跨上下文通信中的执行流程
  - bloom: evaluate
    objective: 评估不同窗口通信方案的适用场景与安全风险，选择最优策略
  - bloom: create
    objective: 设计一个生产级 SPA 路由与窗口管理体系，集成 history、postMessage 与权限控制
exercises:
  - type: fill-blank
    bloom: remember
