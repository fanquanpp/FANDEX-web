...
learningObjectives:
  - bloom: remember
    objective: 复述 Java 泛型的类型擦除（type erasure）机制及其在 JLS 第 4 章中的规范定义，识别 Pizza/GJ 到 JSR 14 的演进时间线
  - bloom: understand
    objective: 解释上界通配符（? extends T）与下界通配符（? super T）的子类型语义，理解 PECS 原则背后的存在量词逻辑
  - bloom: apply
    objective: 运用边界类型参数（bounded type parameters）实现 F-bounded 多态，编写类型安全的容器与算法
  - bloom: analyze
    objective: 分析桥接方法（bridge methods）的生成机制，识别其与协变返回类型、泛型方法重写的交互关系
  - bloom: evaluate
    objective: 评估 Java 类型擦除与 C# reified 泛型、Kotlin 具体化泛型、Scala 泛型、Go 泛型在二进制兼容性、性能、类型安全上的权衡
  - bloom: create
    objective: 设计一个类型安全的异构容器（typesafe heterogeneous container），使用 Class<T> 作为键并规避堆污染（heap pollution）
exercises:
  - id: ex-generics-01
    type: fill-blank
  ...
  - id: ex-generics-05
    ...
    language: java
    fixedCode: |
      // 使用 PECS 原则：src 是生产者用 extends，dest 是消费者用 super
