...
learningObjectives:
  - bloom: remember
    objective: 复述 Java 枚举在 JSR 201 中的引入背景，识别 Joshua Bloch 的「类型安全枚举模式」与 Java 5 enum 的关系
  - bloom: understand
    objective: 解释枚举在编译期被编译器翻译为 `java.lang.Enum` 子类的字节码生成机制，理解 `values()` 与 `valueOf()` 的合成方法原理
  - bloom: apply
    objective: 运用抽象方法 + 实例级重写实现状态机模式，编写支持策略模式的枚举类型
  - bloom: analyze
    objective: 分析 `EnumSet` 与 `EnumMap` 的位向量实现，评估其在性能上相比 `HashSet` 的优势
  - bloom: evaluate
    objective: 评估 Java 注解的三种保留策略（SOURCE/CLASS/RUNTIME）的适用场景，对比 Java 注解与 C# Attribute、Kotlin 注解、Python 装饰器的设计哲学差异
  - bloom: create
    objective: 设计一个自定义注解处理器，在编译期生成样板代码（如 Builder、Equals/HashCode），并与 Lombok、AutoValue 等开源方案对比
exercises:
  - id: ex-enum-anno-01
    type: fill-blank
