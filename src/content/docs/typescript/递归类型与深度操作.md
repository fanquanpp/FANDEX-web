...
learningObjectives:
  - bloom: remember
    objective: 记住 TypeScript 递归条件类型的基本语法与 TypeScript 4.5 引入的尾递归优化机制
  - bloom: understand
    objective: 理解递归类型的求值模型、深度限制（约 50 展开层）与复杂度上限（约 1000 展开次数）的工程动机
  - bloom: apply
    objective: 在工程中运用递归条件类型实现 DeepReadonly、DeepPartial、DeepRequired、DeepPick、DeepOmit 等深度类型操作工具
  - bloom: analyze
    objective: 分析递归类型的求值过程、尾位置的识别规则，预测递归类型在给定输入下的展开结果与是否会触发深度错误
  - bloom: evaluate
    objective: 评估递归类型在大型代码库中的编译性能影响，权衡尾递归优化、累加器模式与拆分类型策略，决定何时回退到运行时校验
  - bloom: create
    objective: 设计 JSON 类型推导器、Schema 派生器、循环引用安全类型、深度路径键类型，覆盖 0 基础学习者至高级类型工程师的全谱系需求
exercises:
  fill-blank:
