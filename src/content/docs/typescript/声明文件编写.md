...
learningObjectives:
  - bloom: remember
    objective: 记住 .d.ts 文件的基本语法、三斜线指令、declare 关键字的五种形式（const/var/let/function/class/namespace/module/global）
  - bloom: understand
    objective: 理解声明合并（Declaration Merging）的四种形式（接口/命名空间/函数/模块扩展）与冲突解决规则
  - bloom: apply
    objective: 在工程中编写符合 DefinitelyTyped 规范的第三方库声明文件，包括 ESM/CJS 双格式入口与子路径导出
  - bloom: analyze
    objective: 分析模块解析策略与声明文件查找路径的关系，预测给定 import 语句对应的 .d.ts 文件位置
  - bloom: evaluate
    objective: 评估一个 npm 包的声明文件组织方式（bundled/types/separate @types）的优劣，决定何时使用全局类型增强与模块扩展
  - bloom: create
    objective: 设计一个完整的库类型声明方案，覆盖 UMD 全局访问、模块扩展、命名空间合并、泛型默认值与条件类型导出
exercises:
  fill-blank:
