...
learningObjectives:
  - level: remember
    objective: 复述类型论发展史，从 Simply Typed Lambda Calculus 到 System F 再到 Hindley-Milner 的演进脉络
  - level: understand
    objective: 解释结构类型与名义类型的语义差异，并能用 KaTeX 写出宽度子类型与深度子类型的形式规则
  - level: apply
    objective: 运用协变、逆变、双变、不变规则预测函数类型、数组、Promise 的子类型关系
  - level: analyze
    objective: 拆解 TypeScript 编译器的类型推导算法，分析控制流分析中的窄化与类型守卫
  - level: evaluate
    objective: 评估条件类型、映射类型、模板字面量类型在类型级计算上的表达力与图灵完备性
  - level: create
    objective: 基于同伦类型论思想设计类型级证明，构造类型安全的 DSL 与运行时验证集成方案
exercises:
  fill-blank:
