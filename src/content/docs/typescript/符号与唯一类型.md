...
learningObjectives:
  - bloom: understand
    objective: 理解 Symbol 在 JavaScript 中的运行时唯一性与 TypeScript 中 unique symbol 的编译期唯一性差异
  - bloom: apply
    objective: 运用 unique symbol 实现品牌类型（branded type）与不透明类型（opaque type），模拟名义类型系统
  - bloom: analyze
    objective: 分析 well-known symbols（Symbol.iterator、Symbol.asyncIterator、Symbol.hasInstance 等）的协议机制与元编程能力
  - bloom: evaluate
    objective: 评估 unique symbol 在泛型约束、映射类型、模板字面量类型中的局限性，选择合适的替代方案
  - bloom: create
    objective: 设计基于 Symbol 的依赖注入容器、协议抽象、插件系统等高级架构模式
  - bloom: analyze
    objective: 对比 TypeScript 结构子类型与 Rust/Scala/Haskell 名义子类型在类型安全与可组合性上的差异
exercises:
  fill-blank:
