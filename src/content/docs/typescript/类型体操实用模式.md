...
learningObjectives:
  - bloom: analyze
    objective: 分析条件类型与映射类型的代数性质，建立类型函数的组合与分解思维
  - bloom: evaluate
    objective: 评估类型递归的深度限制与复杂度边界，识别会导致编译器栈溢出或性能崩塌的退化模式
  - bloom: apply
    objective: 运用 infer 与分布式条件类型实现集合运算、元组变换、对象重组等生产级类型工具
  - bloom: create
    objective: 设计并实现 DeepReadonly、TupleToUnion、Path、Get 等经典类型体操题解，并能写出对应的形式推导
  - bloom: understand
    objective: 理解 TypeScript 类型系统的图灵完备性来源与有限性约束，区分类型层与运行时层
  - bloom: analyze
    objective: 对比 TypeScript 类型系统与 Haskell 类型类、Rust trait、Scala 3 given/using 的差异
exercises:
  fill-blank:
