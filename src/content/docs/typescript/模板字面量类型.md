...
learningObjectives:
  - bloom: remember
    objective: 记住模板字面量类型的基本语法与 Uppercase/Lowercase/Capitalize/Uncapitalize 四个内置字符串操作类型的语义
  - bloom: understand
    objective: 理解 TypeScript 4.1 引入模板字面量类型的历史动机，以及它与运行时模板字符串在语义层与编译层的差异
  - bloom: apply
    objective: 在工程中运用 infer 与模板字面量类型实现字符串前缀/后缀提取、事件名构造、CSS 属性派生、路由参数解析等类型推导
  - bloom: analyze
    objective: 分析 TypeScript 字符串模式匹配中的贪婪策略、联合分发与递归终止条件，预测复杂类型表达式的求值结果
  - bloom: evaluate
    objective: 评估模板字面量类型在大型代码库中的编译性能影响，权衡类型复杂度与可维护性，决定何时回退至运行时校验
  - bloom: create
    objective: 设计类型安全的 SQL 查询构造器、HTTP 路由表、CSS-in-JS 主题系统，覆盖 0 基础学习者至高级类型工程师的全谱系需求
exercises:
  fill-blank:
