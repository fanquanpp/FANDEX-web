...
learningObjectives:
  - bloom: remember
    objective: 记忆 Kysely、Drizzle、Prisma 三个类型安全 ORM 的发布年份、原作者与设计动机
  - bloom: understand
    objective: 理解 Row 类型推导的原理——SQL SELECT 列表到 TypeScript 类型的映射规则
  - bloom: apply
    objective: 运用 TypeScript 类型系统构建类型安全的查询构建器，覆盖 SELECT/INSERT/UPDATE/DELETE 四类基础操作
  - bloom: analyze
    objective: 分析编译时类型与运行时数据之间的对齐机制，识别 schema 漂移、列类型不匹配等问题的根源
  - bloom: evaluate
    objective: 评估 Kysely、Drizzle、Prisma 在类型推导精度、运行时性能、迁移工具链、生态成熟度上的工程权衡
  - bloom: create
    objective: 设计并实现一个支持 JOIN、子查询、CTE 的类型安全查询构建器，给出 Row 类型的形式化推导
exercises:
  - id: ex-db-01
    type: fill-blank
