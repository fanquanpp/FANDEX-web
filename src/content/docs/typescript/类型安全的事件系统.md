...
learningObjectives:
  - bloom: understand
    objective: 理解事件驱动范式相对于命令式调用的优劣，掌握发布订阅模式在类型系统中的形式化定义
  - bloom: apply
    objective: 运用条件类型与映射类型构建类型安全的 EventEmitter，使事件名与负载在编译期一一对应
  - bloom: analyze
    objective: 分析同步事件与异步事件在错误传播、背压、取消语义上的差异，选择合适的执行模型
  - bloom: evaluate
    objective: 评估 once、off、wildcard、命名空间、元数据等高级特性对类型系统复杂度的影响
  - bloom: create
    objective: 设计基于 Builder 模式的事件注册器，支持链式声明、严格校验与运行时零开销
  - bloom: analyze
    objective: 对比 EventEmitter、RxJS Observable、Node.js EventEmitter、EventTarget 等不同实现的设计取舍
exercises:
  fill-blank:
