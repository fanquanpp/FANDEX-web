...
learningObjectives:
  - bloom: remember
    objective: 复述 singleton types（单例类型）的类型论定义，识别字面量类型在 TypeScript 类型系统中的位置
  - bloom: understand
    objective: 解释联合类型的代数性质（吸收律、分配律、结合律），理解联合类型与和类型（sum type）的等价关系
  - bloom: apply
    objective: 实现可辨识联合（discriminated union）模式，运用 kind 字段实现类型收窄与穷尽性检查
  - bloom: analyze
    objective: 分析 TypeScript 控制流分析算法（CFA）在类型收窄中的角色，识别 typeof / instanceof / in / truthy / 自定义守卫的适用边界
  - bloom: evaluate
    objective: 评估 never 类型在穷尽性检查中的关键作用，对比 TypeScript 与 Rust / Haskell / Scala 3 的穷尽性保证机制
  - bloom: create
    objective: 设计一个基于可辨识联合的状态机（FSM），覆盖所有状态转换的编译期验证，并实现类型安全的模式匹配
exercises:
  - id: ex-literal-01
    type: fill-blank
