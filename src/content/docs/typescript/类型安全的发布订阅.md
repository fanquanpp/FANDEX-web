...
learningObjectives:
  - bloom: remember
    objective: 复述观察者模式与发布订阅模式的差异，识别 GoF 设计模式原始论文中的核心定义
  - bloom: understand
    objective: 解释 TypeScript EventMap 设计中协变与逆变的角色，理解函数参数位置上的双变妥协带来的类型安全漏洞
  - bloom: apply
    objective: 实现一个生产级类型安全 PubSub，覆盖 subscribe/once/off/emit/onceWithPriority 等接口，并保证 unsubscribe 类型精确
  - bloom: analyze
    objective: 分析 Node.js EventEmitter、RxJS Subject、VS Code Emitter 与自研 PubSub 在类型签名上的差异，识别各自的类型安全边界
  - bloom: evaluate
    objective: 评估事件循环顺序、错误处理策略、内存泄漏风险对生产环境发布订阅系统的综合影响，给出权衡决策
  - bloom: create
    objective: 设计一个支持事件过滤、优先级队列、命名空间、生命周期管理的可扩展类型安全事件总线架构
exercises:
  - id: ex-pubsub-01
    type: fill-blank
  ...
  - id: ex-pubsub-05
    ...
    buggyCode: |
      once<K extends keyof Events & string>(event: K, handler: (p: Events[K]) => void) {
