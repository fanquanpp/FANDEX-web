...
learningObjectives:
  - bloom: remember
    objective: 记住协变、逆变、不变与双变四种型变关系的基本定义与符号表示
  - bloom: understand
    objective: 理解函数类型的子类型化规则 S <: T 推导出的参数位置逆变与返回位置协变
  - bloom: apply
    objective: 在 TypeScript 工程中正确配置 strictFunctionTypes，并运用 in/out 修饰符标注泛型型变方向
  - bloom: analyze
    objective: 分析数组协变与 React props 逆变在真实代码中的类型安全隐患
  - bloom: evaluate
    objective: 评估 TypeScript 选择双变默认行为与严格逆变检查之间的工程权衡
  - bloom: create
    objective: 设计类型安全的回调注册器与事件派发系统，规避型变陷阱
exercises:
  fill-blank:
