...
learningObjectives:
  - bloom: remember
    objective: 记住 .d.ts 文件的基本结构与三种模块声明形式（namespace/module/global）
  - bloom: understand
    objective: 理解 TypeScript 的四种模块解析策略（classic/node/node16/nodenext/bundler）的差异与适用场景
  - bloom: apply
    objective: 在 tsconfig.json 中正确配置 paths/exports/types/typeRoots，并书写符合 NodeNext 规范的导入路径
  - bloom: analyze
    objective: 分析 ESM 与 CJS 互操作中 esModuleInterop/allowSyntheticDefaultImports 的运行时与类型层差异
  - bloom: evaluate
    objective: 评估一个 npm 包的 package.json exports 字段是否符合 TypeScript 子路径导出与类型解析要求
  - bloom: create
    objective: 设计一个 monorepo 项目的模块解析策略，覆盖前后端共享类型、子路径导出与多入口声明
exercises:
  fill-blank:
