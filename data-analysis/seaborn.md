...
**输出说明**：`kde=True` 在直方图上叠加核密度估计曲线，同时展示离散频数和连续密度。直方图显示总账单呈右偏分布，多数集中在 10-25 美元。
...
**输出说明**：KDE（核密度估计）是直方图的平滑版本，不受 bins 选择的影响。`fill=True` 填充曲线下方区域。KDE 更适合比较不同组的分布形态。
...
**输出说明**：小提琴图是箱线图与核密度估计的结合。`split=True` 将两个组分别显示在小提琴的两侧，便于对比。`inner='quartile'` 在内部显示四分位数线。小提琴的宽度表示该位置的密度。
