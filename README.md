...
[![CI](https://img.shields.io/badge/CI-8阶段-22c55e?style=flat-square&logo=githubactions&logoColor=white)](./.github/workflows/ci.yml)
...
| 文档完整   | 1995 篇文档覆盖 51 个模块，文档体系最为完善，是内容最齐全的版本    |
...
> 1995 篇文档 · 51 个模块 · 27 篇术语表 · 交互式测验 · 知识地图
...
| 层级     | 技术                           | 说明                                                                                             |
| :------- | :----------------------------- | :----------------------------------------------------------------------------------------------- |
| 框架     | Astro 7                        | 静态站点生成 (SSG)，岛屿架构，内容集合 (Content Collections)                                     |
| 交互     | Vue 3                          | `client:load` / `client:visible` 按需水合                                                        |
| 样式     | Tailwind CSS v4                | CSS-first 配置，原子化工具类，Design Tokens 桥接                                                 |
| 组件     | shadcn-vue (radix-vue)         | 无障碍基础组件库（Button / Card / Dialog / Tabs 等 8 个）                                        |
...
| 代码运行 | Pyodide (Python) + Web Worker  | JS/TS 沙箱执行 + Python 在浏览器内执行（按需加载）                                               |
...
| 测试     | Vitest 4 + @vitest/coverage-v8 | 单元测试 + V8 覆盖率                                                                             |
| 质量     | Husky + lint-staged + Prettier | Pre-commit 自动格式化 + 类型检查                                                                 |
| 版本     | Changesets                     | PR 驱动的版本声明，自动生成 CHANGELOG                                                            |
| CI/CD    | GitHub Actions                 | 8 阶段流水线 (lint → type-check → unit-test → e2e-test → build → qa-check → lighthouse → deploy) |

## 快速开始

```bash
# 安装依赖（要求 Node.js 22 LTS）
npm install
...
FANDEX-Web/
├── .github/                  # GitHub 配置
│   ├── workflows/            #   CI/CD 流水线（ci.yml 8 阶段、changeset.yml PR 检查）
│   ├── ISSUE_TEMPLATE/       #   Issue 模板（bug / feature / content_improvement）
│   ├── PULL_REQUEST_TEMPLATE.md
│   ├── CODEOWNERS            #   代码所有者审查规则
│   └── dependabot.yml
├── .changeset/               # Changesets 版本管理
│   ├── config.json
│   └── README.md
├── docs/                     # 开发者文档
│   ├── architecture.md       #   架构总览与分层规则
│   ├── contributing.md       #   贡献指南与流程
│   ├── design-system.md      #   设计系统与 Design Tokens
│   └── CHANGELOG.md          #   变更日志（由 Changesets 维护）
├── public/                   # 静态资源
├── scripts/                  # 构建与质量脚本
│   ├── build-glossary-index.mjs
│   ├── build-search-index.mjs
│   ├── analyze-bundle.mjs    #   Bundle 体积预算校验
│   └── qa-check.mjs          #   发布前质量检查
├── src/
│   ├── components/           # Astro 组件
│   ├── ui/                   #   shadcn-vue 基础组件
│   ├── content/              #   内容集合（docs / glossary / roadmap）
│   │   └── docs/             #     1995 篇文档源（Markdown/MDX）
│   ├── islands/              #   Vue 岛屿组件（client:load / client:visible）
│   ├── layouts/              #   页面布局
│   ├── pages/                #   路由页面
│   ├── services/             #   Service 层（10 个业务服务，单一职责）
│   ├── data/                 #   Data 层（数据存取与索引）
│   ├── types/                #   TypeScript 类型定义
│   ├── utils/                #   纯函数工具（无副作用）
│   └── styles/               #   全局样式与 Design Tokens
├── tests/                    # 单元测试
├── astro.config.ts           # Astro 配置（Shiki / MDX / sitemap / Pagefind）
├── components.json           # shadcn-vue 配置
├── lighthouse-budget.json    # Lighthouse CI 性能预算
├── tailwind.config.ts
├── tsconfig.json
└── package.json
```

> 完整架构说明（三层架构 / 服务依赖矩阵 / 扩展指南）详见 [docs/architecture.md](./docs/architecture.md)。

## 部署

仓库已配置 GitHub Actions 自动部署（`.github/workflows/ci.yml`），push 到 `main` 分支即自动触发 8 阶段流水线，全部通过后自动部署至 GitHub Pages。

**部署流程：**

1. **CI 流水线（8 阶段，全部须通过）**
   - `lint` — Prettier 格式校验 + remark 文档规范
   - `type-check` — astro check TypeScript 类型检查
   - `unit-test` — Vitest 单元测试
   - `e2e-test` — 构建产物冒烟测试（6 条关键路由）
   - `build` — Astro 静态构建 + Pagefind 索引 + Bundle 体积校验
   - `qa-check` — 发布前质量检查（18 项检查点）
   - `lighthouse` — Lighthouse CI 性能预算（LCP ≤ 2.5s / CLS ≤ 0.1 / TBT ≤ 200ms）
   - `deploy` — 上传 Pages 产物并部署

2. **Pages 配置**
   - 仓库 Settings → Pages → Source 选择 **GitHub Actions**

3. **版本发布**
   - PR 必须附带 `.changeset/*.md` 声明（由 `changeset.yml` 自动检查）
   - 合并至 `main` 后维护者执行 `npm run changeset:version` 更新版本号与 `docs/CHANGELOG.md`
...
| 文档                                             | 说明                                                       | 适用对象             |
| :----------------------------------------------- | :--------------------------------------------------------- | :------------------- |
| [docs/architecture.md](./docs/architecture.md)   | 架构总览、三层架构规则、服务依赖矩阵、扩展指南             | 所有贡献者           |
| [docs/contributing.md](./docs/contributing.md)   | 贡献流程、分支规范、提交规范、代码风格、审查标准           | 所有贡献者           |
| [docs/design-system.md](./docs/design-system.md) | Design Tokens、配色系统、字体、间距、动画、shadcn-vue 组件 | UI/样式贡献者        |
| [docs/CHANGELOG.md](./docs/CHANGELOG.md)         | 变更日志（由 Changesets 自动维护）                         | 所有关注版本演进者   |
| [.changeset/README.md](./.changeset/README.md)   | Changesets 流程说明与版本规则映射                          | 所有提交 PR 的贡献者 |
| [CONTRIBUTING.md](./CONTRIBUTING.md)             | 简版贡献指引（根目录索引）                                 | 新贡献者             |
...
| [CONTRIBUTING.md](./CONTRIBUTING.md)       | 贡献指南（根目录索引，详见 docs/contributing.md） |
